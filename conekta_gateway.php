...
function conektacheckout_order_status_completed($order_id = null)
{
  ...
  if($authcap)
  {
    Conekta::setApiKey(get_post_meta( $order_id, 'key', true));
    try
    {
      $tid = get_post_meta( $order_id, 'transaction_id', true);
      $ch = Conekta_Charge::retrieve($tid);
      if($total < $ch->amount)
      {
          $params['amount'] = $total;
      }
    ...
    catch(Conekta_Error $e)
    {
        ...
        error_log('Gateway Error:' . $error_description . "\n");
        $woocommerce->add_error(__('Payment error:', 'woothemes') . $error_description);
      return null;
    }
   return true;
  }
